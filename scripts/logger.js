/*
  @source https://github.com/NYTimes/kyt/blob/f061d6d32820c3cde13c1748f647d7efd9c4af7e/cli/logger.js
 */
...
const write = (status, text, verbose) => {
  ...
  if (status === 'error') line = cl.red;
  if (status === 'start') line = cl.white;
  if (status === 'warn') line = cl.yellow;
  if (status === 'task') line = cl.green;
...
// Starting a process
const start = text => write('start', text);

// Ending a process
const end = text => write('end', text);

// Tasks within a process
const task = text => write('task', text);
...
// Verbose output
// takes optional data
const debug = (text, data) => write('debug', text, data);

// Warn output
const warn = (text, data) => write('warn', text, data);

// Error output
// takes an optional error
const error = (text, err) => write('error', text, err);
